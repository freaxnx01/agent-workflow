...
open_draft_pr() {
  ...
  if [[ "$ec" -eq 0 ]]; then
    ...
    # The numeric guard matters: on the `exists` and `failed` paths `out` is an
    # error message, not a URL, so without it a fragment of prose would travel
    # into $GITHUB_OUTPUT as the PR number.
...
#    GITHUB_OUTPUT is cleared for the child: find-pipeline-pr.sh writes its own
#    found= / pr-number= keys there, and inheriting ours meant this step's
#    outputs carried values nothing here had decided (#364).
pr_out="$(ISSUE_NUMBER="$ISSUE_NUMBER" REPO="$REPO" GITHUB_OUTPUT='' \
  PIPELINE_PRS_JSON="${PIPELINE_PRS_JSON:-}" bash "$HERE/find-pipeline-pr.sh" 2>/dev/null || printf 'found=false')"
if [[ "$pr_out" == *"found=true"* ]]; then
  # find-pipeline-pr.sh already reports the number it selected; reuse it rather
  # than asking GitHub a second question with a second chance to disagree.
  PR_NUMBER_OUT="$(printf '%s\n' "$pr_out" | sed -n 's/.*pr-number=\([0-9]*\).*/\1/p' | head -1)"
